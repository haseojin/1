...
<?php
	...
	$query="select count(*) from p_bbs";
	//mysql_query("set names utf8");  //언어셋 utf8
	$result=  mysqli_query($con, $query);
	$temp= mysqli_fetch_array($result);
	$totals= $temp[0];

	$sql2= "select * from p_bbs  order by no desc limit $page, $view_total";
	...
	$sql2 = "SELECT * FROM p_bbs WHERE ".$_GET['Search_mode']." LIKE '%".$_GET['Search_text']."%'   order by no desc limit $page, $view_total";
	$result2 = mysqli_query($con, $sql2); 

	$query="select count(*) from p_bbs WHERE ".$_GET['Search_mode']." LIKE '%".$_GET['Search_text']."%'  limit $page, $view_total";
	//mysql_query("set names utf8");  //언어셋 utf8
	$result=  mysqli_query($con, $query);
	$temp= mysqli_fetch_array($result);
	$totals= $temp[0];
