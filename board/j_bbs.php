...
<?php
	...
	$view_total = 10; //한 페이지에 20개 게시글이 보인다.
	if(!$_page)($_page=1); //페이지 번호가 지정이 안되었을 경우
	$page= ($_page-1)*$view_total;

	$query="select count(*) from j_bbs";
	//mysql_query("set names utf8");  //언어셋 utf8
	$result=  mysqli_query($con, $query);
	$temp= mysqli_fetch_array($result);
	$totals= $temp[0];

	$sql2= "select * from j_bbs  order by no desc limit $page, $view_total";
	...
	$sql2 = "SELECT * FROM j_bbs WHERE $_GET['Search_mode'] LIKE '%$_GET['Search_text']%'   order by no desc limit $page, $view_total";
	$result2 = mysqli_query($con, $sql2); 

	$query="select count(*) from j_bbs WHERE $_GET['Search_mode'] LIKE '%$_GET['Search_text']%'  limit $page, $view_total";
	//mysql_query("set names utf8");  //언어셋 utf8
	$result=  mysqli_query($con, $query);
	$temp= mysqli_fetch_array($result);
	$totals= $temp[0];
...
<section>
	...
	<table id="list_table">  
		...
		<tr id="list_table_content"> 
			...
			?>
				<!--<a href='#' onclick=location.href='./c_view.php?no=$row[no]'>-->
